...
class kittiDataset(base_dataset.BaseDataset):
    ...
    def _get_frame_from_anno(self, anno):
        ...
        if self.coordinate_mode == 'velodyne':
            ...
            l.append(anno["length"])
            w.append(anno["width"])
            h.append(anno["height"])
        ...
        # # todo add image
        # print(bb.orientation.degrees * bb.orientation.axis[-1],bb.orientation.axis, bb.orientation.degrees, anno["rotation_y"])
        # from .open3d_vis_utils import plt_scenes
        # plt_scenes(pc.points.T, gt_boxes=bb.corners().T.reshape(1,8,3))

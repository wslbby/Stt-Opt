...
def draw_gt_boxes3d(gt_boxes3d, fig, color=[(1,0,0),(0,1,0),(0,0,1)], line_width=2, center=None) -> object:


    num = len(gt_boxes3d)
    
    for b, color in zip(gt_boxes3d, color):
        b = b.squeeze(0)
        for k in range(0,4):
    ...
    if labels is not None:
        ...
        if new_points is not None:
            if flow is not None:
                ...
                mlab.points3d(new_points[:, 0], new_points[:, 1], new_points[:, 2], color=(0, 0, 0), scale_factor=0.12)
                #print(flow)
                # flow[:,[1,2]] = flow[:,[2,1]]
                wrapped = points + flow
                ...
                mlab.points3d(new_points[:, 0], new_points[:, 1], new_points[:, 2], color=(0, 0, 0), scale_factor=0.08)
        else:
            mlab.points3d(points[:, 0], points[:, 1], points[:, 2], color=(0, 0, 0), scale_factor=0.08)
...
def mlab_trace(pred, gt):
    if isinstance(pred, torch.Tensor):
        pred = pred.cpu().numpy()
    if isinstance(gt, torch.Tensor):
        gt = gt.cpu().numpy()
    fig = mlab.figure(bgcolor=(1, 1, 1), size=(1200, 800))
    for i, (pred_bbox, gt_bbox) in enumerate(zip(pred, gt)):
        mlab.points3d(pred_bbox[0], pred_bbox[1], pred_bbox[2], color=(0, 0, 0), scale_factor=0.08)
        mlab.points3d(gt_bbox.center[0], gt_bbox.center[1], gt_bbox.center[2], color=(1, 0, 0), scale_factor=0.08)
...
def plt_consecutive_frames(pcs):
    fig = mlab.figure(bgcolor=(1, 1, 1), size=(1200, 800))
    for pc in pcs:
        pc = pc.points.T
        mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], color=(0, 0, 0), scale_factor=0.06)
